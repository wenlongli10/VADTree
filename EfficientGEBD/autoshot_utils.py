...
def get_frames(fn, width=48, height=27):
    ...
    return video
def get_frames_opencv(video_fn, input_size=(27,48,3)):
    h = input_size[0]  # 27
    w = input_size[1]  # 48
    c = input_size[2]  # 3
    ...
    for i in range(videolen):
        ...
        frames.append(frame)
    frames = np.array(frames).reshape([-1, h, w, c])
    return frames

def get_frames_opencv_trans(video_fn, input_size=(27,48,3), Trans=None,):
    h = input_size[0]  # 27
    w = input_size[1]  # 48
    c = input_size[2]  # 3
    cap = cv2.VideoCapture(video_fn)
    ...
    for i in range(videolen):
        ...
        frame = frame[:, :, ::-1]
        frame = cv2.resize(frame, (w, h))
        # frames.append(Trans(frame)[None,...])
        frames.append(Trans(frame))

        org_frames.append(cv2.resize(frame, (96, 54)))

    frames = torch.stack(frames)

    return frames, np.array(org_frames)

def get_100frames_opencv_trans(video_fn, input_size=(27,48,3), Trans=None,):
    h = input_size[0]  # 27
    w = input_size[1]  # 48
    c = input_size[2]  # 3
    cap = cv2.VideoCapture(video_fn)
    ...
    frames = []
    org_frames = []
    idx_100 = np.linspace(0, videolen-1, 100, dtype=int)
    ...
    return frames, np.array(org_frames), videolen, idx_100, fps

def get_split_100frames_opencv_trans(video_fn, input_size=(224,224,3), Trans=None,):
    h = input_size[0]  # 27
    w = input_size[1]  # 48
    c = input_size[2]  # 3
    cap = cv2.VideoCapture(video_fn)
    ...
    frames = []
    org_frames = []
    idx_split_100 = np.linspace(0, videolen-1, sample_frames, dtype=int)

    for i in range(videolen):
        ret = cap.grab()
        if ret == False:
            print(f'{video_fn} {i} video read error, maybe video broken')
            continue
        ret, frame = cap.retrieve()
        if i in idx_split_100:
            frame = frame[:, :, ::-1]
            frame = cv2.resize(frame, (w, h))
            frames.append(Trans(frame).to(torch.float16))
            org_frames.append(cv2.resize(frame, (96, 54)))

    frames = torch.stack(frames)
    if  len(frames)<sample_frames-1: # MSAD的Assault_8最后一帧坏掉了,只有少数帧损坏，可略过
        print(f"video {video_fn} sampling frames error, expect {sample_frames} frames, but got {len(frames)} frames, video maybe broken")
        videolen = len(frames)
        idx_split_100 = np.linspace(0, videolen-1, len(frames), dtype=int)
    return frames, np.array(org_frames), videolen, idx_split_100, fps
...
#             color[(j + 1) % 3] = 255
    ...
    return img
def visualize_predictions_TransNetv2(frames: np.ndarray, predictions):
    from PIL import Image, ImageDraw, ImageFont
    ...
    for i, pred in enumerate(zip(*predictions)):
        ...
        # 在图像上绘制数字
        draw.text((x-48,y-28), str(num), font=font, fill=(0,255,255))
        # we can visualize multiple predictions per single frame
        for j, p in enumerate(pred): # [绿 蓝]
            color = [0, 0, 0]
            color[(j + 1) % 3] = 255

            value = round(p * (ih - 1))
            if value != 0:
                draw.line((x + j, y, x + j, y - value), fill=tuple(color), width=2)
        num += 1
    # img.save('1.png')
    return img
...
def get_batches(frames):
    ...
    if reminder == 50:
        reminder = 0
    frames = torch.concatenate([frames[:1]] * 25 + [frames] + [frames[-1:]] * (reminder + 25), 0)
...
def predictions_to_scenes_TransNetv2(predictions: np.ndarray, threshold: float = 0.4):
